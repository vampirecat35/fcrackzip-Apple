...
static crackmethod *crack_method = methods;
...
check_unzip (const char *pw)
{
  ...
  pwbuf[0] = '-';
  pwbuf[1] = 'P';
  strlcpy(&pwbuf[2], pw, (sizeof(pwbuf) - 3));
  argv[2] = pwbuf;
  argv[3] = (char *)file_path[0];
  argv[4] = NULL;
...
dictionary_gen (void)
{
   ...
   * even higher win :(
   */
  if (fgets ((char *)pw, MAX_PW+1, dict_file))
    {
      pw[strlen ((const char *)pw) - 1] = 0;
      return -1;
    }
  else
    {
...
main (int argc, char *argv[])
{
  ...
  switch (mode)
    ...
    case m_dictionary:
      ...
      if (!(dict_file = fopen ((const char *)pw, "r")))
        {
          perror ((const char *)pw);
          exit (1);
        }
      else
        {
