  ...
  do
    {
      ...
      {
          printf ("checking pw %s\r", pw);
          fflush (stdout);
      }

      if (changed < 0)
          pw_end = (u8 *)pw + strlen ((const char *)pw);
...
#define P(x,y,c) (UI)dst_pixel((x),(y),(c))
	...
	for (y = transform_height; y--;)
	  {
	    current += (unsigned long)abs ((int)P (x - 1, y, 0) - (int)P (x, y, 0));
	    current += (unsigned long)abs ((int)P (x - 1, y, 1) - (int)P (x, y, 1));
	    current += (unsigned long)abs ((int)P (x - 1, y, 2) - (int)P (x, y, 2));
	...
	for (x = transform_width; x-- && current < minimum;)
	  {
	    current += (unsigned long)abs ((int)P (x, y - 1, 0) - (int)P (x, y, 0));
	    current += (unsigned long)abs ((int)P (x, y - 1, 1) - (int)P (x, y, 1));
	    current += (unsigned long)abs ((int)P (x, y - 1, 2) - (int)P (x, y, 2));
...
overflow:
      if (current < minimum)
	{
	  ...
	  if ((changed = cbfunc ((const char *)pw, (const char *)info)))
	    return changed;
	}
